...
void* Mem_Alloc(int size) {
    ...
    while(curr->packed_pointer != NULL){ // find available block
        if((unsigned)curr->packed_pointer & 1 || curr->size < (unsigned)size){
            curr = (BLOCK_HEADER *)((unsigned)curr->packed_pointer & 0xfffffffe);
            continue;
        }
        else{
            if(curr->size - size < 4 + sizeof(BLOCK_HEADER)){
                curr->packed_pointer = (void *) ((unsigned)curr->packed_pointer + 1);
            }
            else{
                BLOCK_HEADER *new = (BLOCK_HEADER *)((unsigned)curr + sizeof(BLOCK_HEADER) + (unsigned)size);
                new->packed_pointer = curr->packed_pointer;
                curr->packed_pointer = (void *)new + 1; 
                new->size = curr->size - sizeof(BLOCK_HEADER) - (unsigned)size;
                curr->size = size; 
            }
        } 
...
int Mem_Free(void *ptr){
    if ((unsigned)ptr < (unsigned)first_header + sizeof(BLOCK_HEADER) ||
         (unsigned)ptr >= (unsigned)first_header + (unsigned)alloc_size - sizeof(BLOCK_HEADER) || 
         (unsigned)ptr % 4 != 0) return -1;
    BLOCK_HEADER *curr = first_header;
    BLOCK_HEADER *prev = NULL;
    
    while(curr->packed_pointer != NULL){ // iterate all header
        if((unsigned)curr + sizeof(BLOCK_HEADER)!= (unsigned)ptr){
            prev = curr;
            curr = (BLOCK_HEADER *)((unsigned)curr->packed_pointer & 0xfffffffe);
            continue;
        }
        if(((unsigned)curr->packed_pointer & 1) == 0) return -1; // ptr is free

        curr->packed_pointer -= 1;// free the block 
        BLOCK_HEADER *next = (BLOCK_HEADER *)(curr->packed_pointer);

        if(next->packed_pointer != NULL && ((unsigned)next->packed_pointer & 1) == 0){
            curr->packed_pointer = next->packed_pointer;
            curr->size = curr->size + sizeof(BLOCK_HEADER) + next->size;
        }
        if(prev != NULL && ((unsigned)prev->packed_pointer & 1) == 0){
            prev->packed_pointer = curr->packed_pointer; 
            prev->size = prev->size + sizeof(BLOCK_HEADER) + curr->size;
        }
        return 0;
    }
    return -1;
